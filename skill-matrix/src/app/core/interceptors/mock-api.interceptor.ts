...
let notificationsCache: { notificationId: string; userId: string; type: string; message: string; isRead: boolean; date: string; linkTo: string | null }[] | null = null;
...
async function loadNotifications(): Promise<NonNullable<typeof notificationsCache>> {
  if (notificationsCache) return notificationsCache;
  try {
    const response = await fetch('/assets/mock-data/notifications.json');
...
export const mockApiInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  const notifUserMatch = url.match(/^\/api\/notifications\/([^/]+)$/);
  if (notifUserMatch && req.method === 'GET') {
    return handleGetNotifications(notifUserMatch[1]);
  }
...
// ── Feature 009: Notifications Handler ────────────────────────────────────────
function handleGetNotifications(userId: string): Observable<HttpResponse<unknown>> {
  const role = getCurrentUserRole();
  if (!role) {
    return makeError(403, 'Not authenticated.');
  }
  return new Observable<HttpResponse<unknown>>((sub) => {
    loadNotifications().then((allNotifications) => {
      ...
      sub.next(new HttpResponse({ status: 200, body: userNotifications }));
      sub.complete();
    });
  }).pipe(delay(getSimulatedDelay()));
}
